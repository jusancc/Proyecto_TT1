...
tuple<double, double> NutAngles(double Mjd_TT)
{
    ...
    double C[N_coeff][9] = {
        // l   l'  F   D   Om      dpsi      dpsi*T     deps      deps*T
    ...
    double dummy;
    double l = modf((485866.733 + (1325.0 * rev + 715922.633) * T + 31.310 * T2 + 0.064 * T3) / rev, &dummy) * rev;
    double lp = modf((1287099.804 + (99.0 * rev + 1292581.224) * T - 0.577 * T2 - 0.012 * T3) / rev, &dummy) * rev;
    double F = modf((335778.877 + (1342.0 * rev + 295263.137) * T - 13.257 * T2 + 0.011 * T3) / rev, &dummy) * rev;
    double D = modf((1072261.307 + (1236.0 * rev + 1105601.328) * T - 6.891 * T2 + 0.019 * T3) / rev, &dummy) * rev;
    double Om = modf((450160.280 - (5.0 * rev + 482890.539) * T + 7.455 * T2 + 0.008 * T3) / rev, &dummy) * rev;
